...
class NoAvailableServicesWidget extends StatefulWidget {
  final String responseSuggestedServices;
  const NoAvailableServicesWidget(
      {super.key, required this.responseSuggestedServices});

  @override
  State<NoAvailableServicesWidget> createState() =>
      _NoAvailableServicesWidgetState();
}

class _NoAvailableServicesWidgetState extends State<NoAvailableServicesWidget> {
  ...
  Future<void> _getVerifiedShops() async {
    ...
    try {
      ...
      // Check if responseSuggestedServices is 'No applicable services available.'
      final shouldShowAllShops = widget.responseSuggestedServices ==
      ...
      for (var doc in querySnapshot.docs) {
        ...
        // If responseSuggestedServices is not 'No applicable services available.' and no matching service found, skip this shop
        if (!shouldShowAllShops &&
            !widget.responseSuggestedServices
                .split(',')
                .map((service) => service.trim())
                .any((service) => serviceSpecialization.contains(service))) {
          continue;
        }

        String? assetPath;
        for (String service in serviceSpecialization) {
          if (categoryIcons.containsKey(service)) {
            assetPath = categoryIcons[service];
            break;
          }
        }

        if (assetPath == null) {
          // Skip if no relevant icon is found
          continue;
        }
